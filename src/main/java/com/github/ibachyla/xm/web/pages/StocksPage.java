...
/**
 * Page object for the Stocks page.
 */
@Slf4j
@Lazy
@Component
public class StocksPage extends BasePage<StocksPage> {
  ...
  private static final String COUNTRY_FILTER_CSS_LOCATOR_TEMPLATE =
      ".table-country-filter > button[data-value='%s']";
  private static final String STOCK_CELL_CSS_LOCATOR_TEMPLATE = "td[data-xm-qa-name='%s']";

  private static final By SYMBOL_CELL =
      By.cssSelector(STOCK_CELL_CSS_LOCATOR_TEMPLATE.formatted("symbol"));
  ...
  private static final By SPREAD_CELL =
      By.cssSelector(STOCK_CELL_CSS_LOCATOR_TEMPLATE.formatted("minSpread"));
  ...
  /**
   * Checks if the country filter is selected.
   *
   * @param country the country to check
   * @return true if the country filter is selected, false otherwise
   */
  ...
  /**
   * Finds a stock in the table by symbol.
   *
   * @param symbol the symbol of the stock to find
   * @return the stock if found, empty otherwise
   */
  ...
  /**
   * Opens a page for a specific stock by clicking the 'Read More' link.
   * If a stock is not found, an {@link NoSuchElementException} exception is thrown.
   *
   * @param symbol the symbol of the stock to open
   */
